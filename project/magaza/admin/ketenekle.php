...
<?php
    ...
    if ($_POST) {
        ...
        if ($foo->uploaded) {
            $foo->file_new_name_body = 'keten_img';
            $foo->process('../image/urunler/');
            if ($foo->processed) {
                $resim_adi = $foo->file_dst_name;
    
                DB::insert("INSERT INTO keten(product_name, detail, price, img_url, link_url) VALUES(?,?,?,?,?)", array(
                    $_POST['product_name'],
                    $_POST['detail'],
                    $_POST['price'],
                    $resim_adi,
                ...
                echo "<script> Swal.fire( { title:'Başarılı!', text:'Başarıyla Eklediniz.', icon:'success', confirmButtonText:'Kapat' }).then((value)=>{
                    if(value.isConfirmed){window.location.href='keten.php?success=1'}})</script>";
            }
            else{
                echo "<script> Swal.fire( { title:'Hata!', text:'Malesef Eklenmedi', icon:'error', confirmButtonText:'Kapat' }).then((value)=>{
                    if(value.isConfirmed){window.location.href='ketenekle.php'}})</script>";
            }
        } else {
            Header("location:ketenekle.php?resimHata=" . @$foo->error);
        }
    }
