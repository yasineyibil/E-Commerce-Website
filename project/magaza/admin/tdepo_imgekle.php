...
<?php
    ...
    if ($_POST) {
        ...
        if ($foo->uploaded) {
            $foo->file_new_name_body = 'depo_img';
            $foo->process('../../Antique-Rach/image/depo-img/');
            if ($foo->processed) {
                $resim_adi = $foo->file_dst_name;
    
                DB::insert("INSERT INTO tdepo_img(href_url, img_url) VALUES(?,?)", array(
                    $_POST['href_url'],
                    $resim_adi,
                ));
                ...
                echo "<script> Swal.fire( { title:'Başarılı!', text:'Başarıyla Eklediniz.', icon:'success', confirmButtonText:'Kapat' }).then((value)=>{
                    if(value.isConfirmed){window.location.href='tdepo_img.php?success=1'}})</script>";
            }
            else{
                echo "<script> Swal.fire( { title:'Hata!', text:'Malesef Eklenmedi', icon:'error', confirmButtonText:'Kapat' }).then((value)=>{
                    if(value.isConfirmed){window.location.href='tdepo_imgekle.php'}})</script>";
            }
        } else {
            Header("location:tdepo_imgekle.php?resimHata=" . @$foo->error);
        }
    }
