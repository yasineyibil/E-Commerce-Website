...
<?php
    ...
    if ($_POST) {
        ...
        if ($foo->uploaded) {
            $foo->file_new_name_body = 'obje_img';
            $foo->process('../image/urunler/');
            if ($foo->processed) {
                $resim_adi = $foo->file_dst_name;
    
                DB::insert("INSERT INTO obje(product_name, detail, price, img_url, link_url) VALUES(?,?,?,?,?)", array(
                    $_POST['product_name'],
                    $_POST['detail'],
                    $_POST['price'],
                    $resim_adi,
                ...
                echo "<script> Swal.fire( { title:'Başarılı!', text:'Başarıyla Eklediniz.', icon:'success', confirmButtonText:'Kapat' }).then((value)=>{
                    if(value.isConfirmed){window.location.href='obje.php?success=1'}})</script>";
            }
            else{
                echo "<script> Swal.fire( { title:'Hata!', text:'Malesef Eklenmedi', icon:'error', confirmButtonText:'Kapat' }).then((value)=>{
                    if(value.isConfirmed){window.location.href='objeekle.php'}})</script>";
            }
        } else {
            Header("location:objeekle.php?resimHata=" . @$foo->error);
        }
    }
