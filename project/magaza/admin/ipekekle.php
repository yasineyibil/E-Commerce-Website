...
if ($_POST) {
    ...
    if ($foo->uploaded) {
        $foo->file_new_name_body = 'ipek_img';
        $foo->process('../image/urunler/');
        if ($foo->processed) {
            $resim_adi = $foo->file_dst_name;

            DB::insert("INSERT INTO ipek(product_name, detail, price, img_url, link_url) VALUES(?,?,?,?,?)", array(
                $_POST['product_name'],
                $_POST['detail'],
                $_POST['price'],
                $resim_adi,
            ...
            echo "<script> Swal.fire( { title:'Başarılı!', text:'Başarıyla Eklediniz.', icon:'success', confirmButtonText:'Kapat' }).then((value)=>{
                    if(value.isConfirmed){window.location.href='ipek.php?success=1'}})</script>";
        } else {
            echo "<script> Swal.fire( { title:'Hata!', text:'Malesef Eklenmedi', icon:'error', confirmButtonText:'Kapat' }).then((value)=>{
                    if(value.isConfirmed){window.location.href='ipekekle.php'}})</script>";
        }
    } else {
        Header("location:ipekekle.php?resimHata=" . @$foo->error);
    }
}
