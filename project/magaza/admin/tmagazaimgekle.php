...
<?php
    ...
    if ($_POST) {
        ...
        if ($foo->uploaded) {
            $foo->file_new_name_body = 'magaza_img';
            $foo->process('../../Antique-Rach/image/magaza-img/');
            if ($foo->processed) {
                $resim_adi = $foo->file_dst_name;
    
                DB::insert("INSERT INTO tmagaza_img(href_url, img_url) VALUES(?,?)", array(
                    $_POST['href_url'],
                    $resim_adi,
                ));
                ...
                echo "<script> Swal.fire( { title:'Başarılı!', text:'Başarıyla Eklediniz.', icon:'success', confirmButtonText:'Kapat' }).then((value)=>{
                    if(value.isConfirmed){window.location.href='tmagaza_img.php?success=1'}})</script>";
            }
            else{
                echo "<script> Swal.fire( { title:'Hata!', text:'Malesef Eklenmedi', icon:'error', confirmButtonText:'Kapat' }).then((value)=>{
                    if(value.isConfirmed){window.location.href='tmagazaimgekle.php'}})</script>";
            }
        } else {
            Header("location:tmagazaimgekle.php?resimHata=" . @$foo->error);
        }
    }
