...
$id = $_GET['id'];
$sorgu = $db->prepare("select * from ipek where id=:id");
...
if ($_POST) {
    ...
    if ($resim_adi == "") {
        ...
        $prm = array(
            'product_name' => $_POST["product_name"], 'detail' => $_POST["detail"],
...
if (@$guncelle) {
    echo "<script> Swal.fire( { title:'Başarılı!', text:'Başarıyla Güncellediniz.', icon:'success', confirmButtonText:'Kapat' }).then((value)=>{
                if(value.isConfirmed){window.location.href='ipek.php'}})</script>";
}
