...
<?php
    $sayfa="Atölye Güncelle";
    include "inc/ahead.php";

    include "../lib/tdb.php";
    /* verileri çekme bölümü */
    $sorgu=$db->prepare("select * from tatolye where id=:id");
    ...
    if($_POST){
        $guncellesorgu=$db->prepare("UPDATE tatolye SET 
        bold_txt=:bold_txt,
        txt=:txt
        WHERE id=:id");
         ...
         if(@$guncelle){
            echo "<script> Swal.fire( { title:'Başarılı!', text:'Başarıyla Güncellediniz.', icon:'success', confirmButtonText:'Kapat' }).then((value)=>{
                if(value.isConfirmed){window.location.href='tatolye.php'}})</script>";
        }
